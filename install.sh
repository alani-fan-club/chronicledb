...
(
  cd "$REPO_DIR"
  # --ignore-scripts: defense-in-depth for the curl|bash distribution path.
  # No first-party package today declares lifecycle scripts, but blocking
  # them at install means a future supply-chain compromise (chronicledb
  # master tamper, or any transitive dep gaining a postinstall) does not
  # automatically execute on user machines.
  npm install --silent --no-audit --no-fund --no-progress --ignore-scripts
)
