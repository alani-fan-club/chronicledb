const { resolveCharacterIds } = require("../../shared/character-identity");

function createCharacterPanelDomain({ getPool, slugify, chatScopedId }) {
  // Per-chat character scoping introduced chr-{chatHash}-{slug} ids
  // alongside the legacy chr-{slug} globals. When a panel query is scoped
  // to a chat, accept either id so per-chat rows are still found by name.
  async function resolveCharIds(pool, characterName, chatId) {
    const idsByName = await resolveCharacterIds(pool, {
      names: [characterName],
      chatIds: chatId ? [chatId] : null,
    });
    const ids = idsByName.get(characterName) || [];
    if (ids.length > 0) return ids;
    const fallback = [slugify(characterName)];
    if (chatId) fallback.push(chatScopedId(characterName, chatId));
    return Array.from(new Set(fallback));
  }

  async function getCharacterPanelStats(settings, characterName, chatId) {
    const p = getPool(settings);
    const charIds = await resolveCharIds(p, characterName, chatId);
      ...
      const { rows } = await p.query(
        `SELECT
           (SELECT COUNT(*) FROM participated_in pi
              ...
              WHERE pi.character_id = ANY($1::text[]) AND e.chat_id = $2)::int AS events,
           (SELECT COUNT(*) FROM traits
              WHERE character_id = ANY($1::text[]) AND source_chat = $2
                AND canonical_id IS NULL)::int AS traits,
           (SELECT COUNT(*) FROM feels_about
              WHERE (from_char = ANY($1::text[]) OR to_char = ANY($1::text[])) AND session_id = $2)::int AS relationships,
           (SELECT MAX(e.message_index) FROM events e
              JOIN participated_in pi ON pi.event_id = e.id
              WHERE pi.character_id = ANY($1::text[]) AND e.chat_id = $2) AS last_seen_turn`,
        [charIds, chatId],
      );
      const r = rows[0] || {};
      return {
    ...
    const { rows } = await p.query(
      `SELECT
         (SELECT COUNT(*) FROM participated_in WHERE character_id = ANY($1::text[]))::int AS events,
         (SELECT COUNT(*) FROM traits
            WHERE character_id = ANY($1::text[]) AND canonical_id IS NULL)::int AS traits,
         (SELECT COUNT(*) FROM feels_about WHERE from_char = ANY($1::text[]) OR to_char = ANY($1::text[]))::int AS relationships,
         (SELECT MAX(e.message_index) FROM events e
            JOIN participated_in pi ON pi.event_id = e.id
            WHERE pi.character_id = ANY($1::text[])) AS last_seen_turn`,
      [charIds],
    );
    const r = rows[0] || {};
    return {
  ...
  async function getCharacterRecentEvents(settings, characterName, limit, chatId) {
    ...
    const sql = chatId
      ? `SELECT e.id, e.summary, e.source_text, e.message_index, e.significance, e.chat_id
         FROM events e
         JOIN participated_in pi ON pi.event_id = e.id
         WHERE pi.character_id = ANY($1::text[]) AND e.chat_id = $3
         ORDER BY e.message_index DESC NULLS LAST, e.timestamp DESC
         LIMIT $2`
      : `SELECT e.id, e.summary, e.source_text, e.message_index, e.significance, e.chat_id
         FROM events e
         JOIN participated_in pi ON pi.event_id = e.id
         WHERE pi.character_id = ANY($1::text[])
         ORDER BY e.message_index DESC NULLS LAST, e.timestamp DESC
         LIMIT $2`;
    const params = chatId ? [charIds, lim, chatId] : [charIds, lim];
    ...
    if (chatId) {
      const { rows } = await p.query(
        `SELECT c.name AS to_name, f.sentiment, f.intensity, f.description, f.updated_at
         ...
         WHERE f.from_char = ANY($1::text[]) AND f.session_id = $2
