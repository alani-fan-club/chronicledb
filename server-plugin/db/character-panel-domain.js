function createCharacterPanelDomain({ getPool, slugify, chatScopedId }) {
  ...
  function resolveCharIds(characterName, chatId) {
    const ids = [slugify(characterName)];
    if (chatId) ids.push(chatScopedId(characterName, chatId));
    return Array.from(new Set(ids));
  }

  async function getCharacterPanelStats(settings, characterName, chatId) {
    const p = getPool(settings);
    const charIds = resolveCharIds(characterName, chatId);
