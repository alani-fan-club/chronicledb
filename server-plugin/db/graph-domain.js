const NODE_SELECTS = {
  characters: "id, name, aliases, description, faction, role, status, significance, first_seen, created_at, updated_at",
  facts: "id, content, domain, confidence, created_at",
  events: "id, summary, source_text, significance, message_index, location_id, chat_id, timestamp, tier, condensed_summary, is_major, world_time",
  locations: "id, name, description, importance, current_state, parent_location, created_at",
  items: "id, name, description, powers, significance, owner_id, location_id, status, created_at, chat_id",
  plotThreads: "id, chat_id, thread_type, title, description, involved_chars, planted_at, resolved_at, importance, created_at, updated_at",
  storyArcs: "id, chat_id, title, description, arc_type, status, importance, start_msg_idx, end_msg_idx, spine_event_id, source, parent_arc_id, hierarchy_level, created_at, updated_at",
};
  ...
  async function traverseFromCharacter(settings, characterName, depth = 3, overrideChatIds) {
    ...
    if (Array.isArray(overrideChatIds) && overrideChatIds.length > 0) {
      ...
      const { rows: chats } = await p.query(
        `WITH n(target) AS (
           SELECT lower(regexp_replace(trim($1::text), '\\s+', ' ', 'g'))
         )
    ...
    //
    // chat_id scope: only seed character rows that belong to a chat in
    // `chatIds` (or are legacy NULL-chat globals). Without this, a chat
    // filter in the UI still seeds character nodes from every other
    // chat — the recursive walk's edge filter excludes their edges but
    // the seed nodes themselves render as orphan stars on the mindmap.
    // The "all chats" path passes chatIds = every chat this character
    // appears in, so the union view stays unchanged.
    const { rows: idRows } = await p.query(
      `WITH n(target) AS (
         SELECT lower(regexp_replace(trim($1::text), '\\s+', ' ', 'g'))
       )
       SELECT DISTINCT id FROM characters c CROSS JOIN n
       WHERE (c.chat_id = ANY($2::text[]) OR c.chat_id IS NULL)
         AND (
           lower(regexp_replace(trim(c.name), '\\s+', ' ', 'g')) = n.target
           OR EXISTS (
             SELECT 1 FROM unnest(c.aliases) a
             WHERE lower(regexp_replace(trim(a), '\\s+', ' ', 'g')) = n.target
           )
         )`,
      [characterName, chatIds],
    );
    ...
    const [
      ...
      // Node hydration.
      charIds.length > 0
        ? p.query(`SELECT ${NODE_SELECTS.characters} FROM characters WHERE id = ANY($1)`, [charIds])
        : EMPTY,
      factIds.length > 0
        ? p.query(`SELECT ${NODE_SELECTS.facts} FROM facts WHERE id = ANY($1)`, [factIds])
        : EMPTY,
      eventIds.length > 0
        ? p.query(`SELECT ${NODE_SELECTS.events} FROM events WHERE id = ANY($1)`, [eventIds])
        : EMPTY,
      locIds.length > 0
        ? p.query(`SELECT ${NODE_SELECTS.locations} FROM locations WHERE id = ANY($1)`, [locIds])
        : EMPTY,
      itemIds.length > 0
        ? p.query(`SELECT ${NODE_SELECTS.items} FROM items WHERE id = ANY($1)`, [itemIds])
        : EMPTY,
      plotIds.length > 0
        ? p.query(`SELECT ${NODE_SELECTS.plotThreads} FROM plot_threads WHERE id = ANY($1)`, [plotIds])
        : EMPTY,
      arcIds.length > 0
        ? p.query(`SELECT ${NODE_SELECTS.storyArcs} FROM story_arcs WHERE id = ANY($1)`, [arcIds])
        : EMPTY,
  ...
  async function getGraphData(settings, { scope, character, chatIds }) {
    ...
    if (connectedIds.size > 0) {
      ...
      const { rows: chars } = await p.query(
        `SELECT ${NODE_SELECTS.characters}
      ...
      const { rows: pts } = await p.query(
        `SELECT ${NODE_SELECTS.plotThreads}
