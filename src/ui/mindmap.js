...
async function showDetailPanel(data) {
  ...
  // Render an empty placeholder; the trait list, source_chat badges, and
  // chat-filter dropdown are populated by renderTraitsSection() after the
  // main panel HTML lands. Splitting it out lets the dropdown re-filter
  // the list in place without rebuilding the whole panel.
  if (data.type === "character") {
    html += `<div id="detail-traits"></div>`;
  }

  content.innerHTML = html;
  if (data.type === "character") {
    renderTraitsSection(title).catch((err) => {
      console.warn("[mindmap] Failed to render traits:", err);
    });
  }
...
// Render the trait list inside the detail panel placeholder. Surfaces
// per-trait `source_chat` so the user can tell which chat each trait came
// from — without this the mindmap (a global cross-chat view) silently
// commingles traits from unrelated stories under one character name.
// Provides a chat-filter dropdown that hides traits not from the chosen
// chat. Filtering is client-side over the already-fetched list, so no
// extra network round-trips.
async function renderTraitsSection(name) {
  const container = document.getElementById("detail-traits");
  if (!container) return;
  container.innerHTML = "";

  let traits;
  try {
    const url = new URL(`${API_BASE}/character/${encodeURIComponent(name)}/traits`, window.location.origin);
    const res = await fetch(url, fetchOpts);
    if (!res.ok) return;
  ...
  // Unique source_chats present in the result set, ordered by trait
  // count descending so the dropdown surfaces the densest chats first.
  const chatCounts = new Map();
  for (const t of traits) {
    const k = t.source_chat || "";
    chatCounts.set(k, (chatCounts.get(k) || 0) + 1);
  }
  const chats = Array.from(chatCounts.entries())
    .sort((a, b) => b[1] - a[1])
    .map(([k]) => k);
  ...
  if (chats.length > 1) {
    html += `<div class="detail-row" style="align-items:center;gap:6px;">`;
    html += `<label class="detail-key" for="trait-chat-filter">Chat</label>`;
    html += `<select id="trait-chat-filter" class="detail-val">`;
    html += `<option value="__all__">All chats (${traits.length})</option>`;
    for (const c of chats) {
      const label = c ? c : "(unknown)";
      html += `<option value="${escapeHtml(c)}">${escapeHtml(label)} (${chatCounts.get(c)})</option>`;
    }
    html += `</select></div>`;
  }
  html += `<ul class="detail-list" id="trait-list">`;
  for (const t of traits) {
    const cat = t.category || "";
    const sc = t.source_chat || "";
    const meta = sc ? `${escapeHtml(cat)} · ${escapeHtml(sc)}` : escapeHtml(cat);
    html += `<li data-source-chat="${escapeHtml(sc)}"><span class="detail-dot" style="background:${traitDotColor(cat)}"></span><span class="detail-item-label">${escapeHtml(t.content)} <span class="detail-meta">${meta}</span></span></li>`;
  }
  html += `</ul>`;
  container.innerHTML = html;

  const filter = document.getElementById("trait-chat-filter");
  if (filter) {
    filter.addEventListener("change", () => {
      ...
      items.forEach((li) => {
        if (v === "__all__") {
          ...
          li.style.display = (li.getAttribute("data-source-chat") || "") === v ? "" : "none";
        }
      });
    });
  }
...
// Populate the topbar chat-filter dropdown with this character's chats.
// `/chats` returns every chat in the DB tagged with its character_name;
// we filter client-side to ones whose character matches the active
// character (case- and whitespace-insensitive). Without this filter the
// dropdown would list every chat in the DB regardless of relevance.
async function populateChatFilter(characterName) {
  const sel = document.getElementById("chat-filter");
  if (!sel) return;
  sel.disabled = true;
  ...
  try {
    ...
    console.warn("[mindmap] chat list fetch failed:", err);
    return;
  }
  const norm = (s) => (s || "").toString().trim().toLowerCase().replace(/\s+/g, " ");
  const target = norm(characterName);
  const mine = chats.filter((c) => norm(c.character) === target);
  if (mine.length === 0) return;
  for (const c of mine) {
    const opt = document.createElement("option");
    opt.value = c.chatId || "";
    opt.textContent = c.chatId || c.label || "(unnamed chat)";
    sel.appendChild(opt);
  }
  sel.disabled = false;
}
...
function initToolbar() {
  // Chat filter: scope the graph to a single chat, or clear to "all chats"
  // (the cross-chat union, which is the default). The dropdown is
  // populated by populateChatFilter() when a character is selected.
  const chatFilter = document.getElementById("chat-filter");
  if (chatFilter) {
    chatFilter.addEventListener("change", async () => {
      activeChatFilter = chatFilter.value || "";
      if (!activeCharacter) return;
      const params = { character: activeCharacter, depth: 3 };
      if (activeChatFilter) params.chat_id = activeChatFilter;
      await loadGraphData("character", params);
    });
  }
