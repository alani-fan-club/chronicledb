...
async function injectMemoryContext() {
  try {
    ...
    // Build recent text for vector search. Slice from the TAIL so the user's
    // current message (and any OOC notes in it) is always included — head-slicing
    // dropped the most recent turn on long chats with verbose info-board AI
    // messages, which is exactly where mention-detection needs to look.
    ...
    if (data.memoryBlock) {
      ctx.setExtensionPrompt(
        ...
        0, // depth — no-op for IN_PROMPT (it always lands at end of system prompt regardless); use IN_CHAT (position 4) with positive depth if you want chat-depth positioning
      );
    }
  } catch (err) {
...
async function mountCharacterPanel() {
  ...
  $("#chronicle_char_panel_clear").on("click", async () => {
    ...
    try {
      const ctx = getContext();
      if (ctx && ctx.chatId) chatId = String(ctx.chatId);
    } catch { /* no active chat context */ }
    const scopeText = chatId ? " in the current chat" : "";
    const ok = window.confirm(`Clear ChronicleDB memories for "${name}"${scopeText}? Traits, relationships, and presence for this character will be deleted. Events themselves are kept.`);
    if (!ok) return;
    try {
      const payload = chatId ? { name, chatId } : { name };
